...
# Force loading only en_core_web_sm model
spacy.load = lambda name=None: original_spacy_load("en_core_web_sm")
from pyresparser import ResumeParser
import nltk
...
connection = pymysql.connect(host='localhost', port=3306, user='root', password='Kartik798', db='cv')
...
def run():
    ...
    link = '[©Developed by Avinish](https://www.linkedin.com/in/avinish-kumar-mahato-3294222a7)'
    ...
    if choice == 'User':
        ...
        if st.button('Login'):
            if ad_user == 'avinish' and ad_password == 'avii1112':
                st.success("Welcome Avinish !")
                # Display Data
